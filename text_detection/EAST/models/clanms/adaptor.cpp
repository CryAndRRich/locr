...
namespace lanms_adaptor {
	vector<vector<float>> polys2floats(const vector<lanms::Polygon> &polys) {
		vector<vector<float>> ret;
		for (size_t i = 0; i < polys.size(); i ++) {
			auto &p = polys[i];
			auto &poly = p.poly;
			ret.emplace_back(vector<float>{
	 ...
	 */
	vector<vector<float>> merge_quadrangle_n9(py::array_t<float, py::array::c_style | py::array::forcecast> quad_n9, float iou_threshold) {
		auto pbuf = quad_n9.request();
		if (pbuf.ndim != 2 || pbuf.shape[1] != 9)
			throw runtime_error("quadrangles must have a shape of (n, 9)");
		auto n = pbuf.shape[0];
