...
namespace lanms {
	...
	// A polygon with a score
	struct Polygon {
		cl::Path poly;
		float score;
	};

	// Compute area of multiple paths
	...
	float poly_iou(const Polygon &a, const Polygon &b) {
		...
		return abs(inter_area) / max(abs(uni_area), 1.0f);
	}
	...
	// Helper class to merge polygons
	class PolyMerger {
		public:
			...
			inline int64_t sqr(int64_t x) { return x * x; }

			Polygon normalize_poly(
					const Polygon &ref,
					const Polygon &p) {

				int64_t min_d = numeric_limits<int64_t>::max();
			...
			Polygon get() const {
				...
				auto &poly = p.poly;
				poly.resize(4);
				auto score_inv = 1.0f / max(1e-8f, score);
	...
	// Standard NMS for polygons
	vector<Polygon> standard_nms(vector<Polygon> &polys, float iou_threshold) {
		size_t n = polys.size();
		if (n == 0)
			return {};
		vector<size_t> indices(n);
		iota(begin(indices), end(indices), 0);
		sort(begin(indices), end(indices), [&](size_t i, size_t j) { return polys[i].score > polys[j].score; });
